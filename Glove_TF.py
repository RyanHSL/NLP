...
class Glove:
    ...
    def fit(self, sentences, cc_matrix=None, learning_rate=1e-4, reg=0.1, xmax=100, alpha=0.75, epochs=10):
        ...
        # Paper call it X, so I will call it X, instead of calling the training data X
        ...
        if not os.path.exists(cc_matrix):
            X = np.zeros((V, V))
            N = len(sentences)
            print("number of sentences to proces:", N)
            it = 0
        ...
        print("max in logs(X): ", logX.max())
        print("time to buld the co-occurence matrix: ", (datetime.now() - t0))
